...
function baidu_get_avatar($uid, $size = 'small')
{
    global $_G;
    $url = avatar($uid, $size, true, false, 1, $_G['setting']['ucenterurl']);
    $headers = get_headers($url);
    if (false === $headers || substr($headers[0], 9, 3) != '200') {
        return $_G['setting']['ucenterurl'] . '/images/noavatar_' . $size . '.gif';
    }
    return $url;
}
