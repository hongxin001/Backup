...
<!--{if $leftside||$_G['forum']['allowside']}-->
<style>
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr>
									...
									<th class="$thread[folder]">
                                    	<em class="view-num view-num-color"><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></em>
										...
										<span class="type-tag">$thread[typehtml]</span>
										<!--{if $thread['moved']}-->
										...
										<!--{/if}-->
                                        <div>
<a href="thread-3566672-1-1.html" class="xi2 reply-num">$thread[allreplies]</a>
<cite class="thread-author"> 
<a href="home.php?mod=space&uid=$thread[authorid]" c="1"{if $groupcolor[$thread[authorid]]} style="color: $groupcolor[$thread[authorid]];"{/if}>$thread[author]</a><!--{if !empty($verify[$thread['authorid']])}--> $verify[$thread[authorid]]<!--{/if}--></cite>
<em>$cmszs_lang[10]&nbsp;<span>$thread[dateline]</span>&nbsp;$cmszs_lang[11]</em><span class="pipe">|</span>
<cite class="thread-author">
<!--{if $thread['lastposter']}--><a href="{if $thread[digest] != -2}home.php?mod=space&username=$thread[lastposterenc]{else}forum.php?mod=viewthread&tid=$thread[tid]&page={echo max(1, $thread[pages]);}{/if}" c="1">$thread[lastposter]</a><!--{else}-->$_G[setting][anonymoustext]<!--{/if}--></cite>
<em>&nbsp;$cmszs_lang[10]&nbsp;<a href="{if $thread[digest] != -2 && !$thread[ordertype]}forum.php?mod=redirect&tid=$thread[tid]&goto=lastpost$highlight#lastpost{else}forum.php?mod=viewthread&tid=$thread[tid]{if !$thread[ordertype]}&page={echo max(1, $thread[pages]);}{/if}{/if}">$thread[lastpost]</a>&nbsp;$cmszs_lang[9]</em>
</div>
									</th>
