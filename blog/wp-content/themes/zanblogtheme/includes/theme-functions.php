...
function zan_register_scripts() {
	...
	wp_register_script( 'flexslider', get_template_directory_uri() . '/ui/flexslider/jquery.flexslider.js', 'jquery', '3.0.1', true );

	// 注册jquery.validate.js
	wp_register_script( 'validate', get_template_directory_uri() . '/ui/js/jquery.validate.js', 'jquery', '3.0.1', true );

  // 注册audiojs.min.js
  wp_register_script( 'audiojs', get_template_directory_uri() . '/ui/audiojs/audio.min.js', 'jquery', '3.0.1', true );

  // 注册shine.min.js
  wp_register_script( 'shine', get_template_directory_uri() . '/ui/js/shine.min.js', 'jquery', '3.0.1', true );

	// 注册zan.js
	wp_register_script( 'zan', get_template_directory_uri() . '/ui/js/zan.js', 'jquery', '3.0.1', true );

	// 注册用户自定义custom.js
	wp_register_script( 'custom', get_template_directory_uri() . '/ui/js/custom.js', 'jquery', '3.0.1', true );
	...
	// 注册zan.css
	wp_register_style( 'zan', get_template_directory_uri() . '/ui/css/zan.css', '', '3.0.1' );

	// 注册用户自定义custom.css
	wp_register_style( 'custom', get_template_directory_uri() . '/ui/css/custom.css', '', '3.0.1' );
...
function zan_archives_list() {
  if( !$output = get_option( 'zan_archives_list' ) ){
    ...
    if ( $year != $year_tmp ) {
      $year = $year_tmp;
      $output .= '<h3 class="zan-year">'. $year .' 年 <small>(注：点击月份可以展开)</small></h3><div class="panel-group" id="accordion">'; 
    }
    if ( $mon != $mon_tmp ) {
      $mon = $mon_tmp;
...
function zan_user_list($avatar_cnt) {
  ...
  $output = '<ul class="readers-list row">'.$output.'</ul>';
  echo $output ;
}
